...
def borrar_actores(request: Request):
    ...
    try:
        ...
        return HTMLResponse(content=f"<h1>Error al obtener actores: {e}</h1>", status_code=500)

    mensaje = request.query_params.get("mensaje", None)
    print(mensaje)
    # Renderizar la plantilla con los datos de actores
    return templates.TemplateResponse("borrar_actores.html", {"request": request, "actores": actores, "mensaje": mensaje})
...
def borrar_directores(request: Request):
    ...
    try:
        ...
        return HTMLResponse(content=f"<h1>Error al obtener directores: {e}</h1>", status_code=500)

    mensaje = request.query_params.get("mensaje", None)
    # Renderizar la plantilla con los datos de directores
    return templates.TemplateResponse("borrar_directores.html", {"request": request, "directores": directores, "mensaje": mensaje})
...
def borrar_director(idDirector: str, request: Request):
    ...
    # Redirigir nuevamente al listado de actores
    return RedirectResponse(url=f"/directores/borrar?mensaje={mensaje}", status_code=303)
