...
def update_genero(db: Session, genero_id: str, genero: schemas.GeneroUpdate):
    genero_query = db.query(models.Genero).filter(models.Genero.id == genero.id).first()
    if genero_query:
        genero_query.nombre=genero.nombre,
        genero_query.descripcion=genero.descripcion

        db.commit()
        db.refresh(genero_query)
    return genero_query 
