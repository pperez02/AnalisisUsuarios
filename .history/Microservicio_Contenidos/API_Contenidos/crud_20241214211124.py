...
# Funcion para obtener todos los subtitulos
def get_all_subtitulos(db: Session):
    subtitulos = (
        db.query(models.Subtitulo).all()
    )
    return subtitulos

# Funcion para obtener todos los doblajes
def get_all_doblajes(db: Session):
    doblajes = (
        db.query(models.Doblaje).all()
    )
    return doblajes
...
def delete_reparto(db: Session, idContenido: str):
    reparto = db.query(models.Reparto).filter(models.Reparto.idContenido == idContenido).all()
    if reparto:
        for item in reparto:
            db.delete(item)
            db.commit()
