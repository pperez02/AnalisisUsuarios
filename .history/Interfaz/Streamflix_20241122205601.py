...
async def pantalla_principal(request: Request, user_id: str):
    ...
    recomendaciones = requests.get(f"{BASE_URL_INTERACCIONES}/usuarios/{user_id}/recomendaciones")
    tendencias = requests.get(f"{BASE_URL_INTERACCIONES}/contenido/tendencias")
    historial = requests.get(f"{BASE_URL_INTERACCIONES}/usuarios/{user_id}/historial")
    generos = requests.get(f"{BASE_URL_CONTENIDOS}/generos")

    if (recomendaciones.status_code != 200) or (tendencias.status_code != 200) or (historial.status_code != 200) or (generos.status_code != 200):
        raise HTTPException(status_code=404, detail="No se pudo recuperar la información precisada") 
